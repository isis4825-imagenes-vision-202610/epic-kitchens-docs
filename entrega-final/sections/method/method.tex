...
\begin{gather}
\widetilde{S}^{(m)}_{ij} = \frac{S^{(m)}_{ij} - \min_k S^{(m)}_{ik}}{\max_k S^{(m)}_{ik} - \min_k S^{(m)}_{ik}}\, \\
...
\begin{gather}
S^{\mathrm{exp}} = \tfrac{1}{2}\,(A_{\mathrm{vv}}\,S^{\mathrm{ens}} + (A_{\mathrm{tt}}\,(S^{\mathrm{ens}})^{\!\top})^{\!\top}),\\
